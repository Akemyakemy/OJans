// Problem: 崭新的假日
// Contest: HDOJ
// URL: https://acm.hdu.edu.cn/contest/problem?cid=1178&pid=1009
...
int cntnum(int m,int d,int l,int r){
	int cnt=0;
	rep(y,l,r){
		if(m==2 && d==29){
			if(!run(y)) continue;
...
inline void solve(){
	...
	print(ans);
	#ifdef debug
		for(int i=0;i<days.size();i++) print(get<0>(days[i]));
	#endif
#endif
}
signed main(){
